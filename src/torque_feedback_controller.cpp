...
TorqueFeedbackController::update(const rclcpp::Time & /*time*/,
  ...
  if (params_.nullspace.projector_type == "dynamic") {
    ...
    Eigen::MatrixXd J_pinv =
        pseudo_inverse(J_, params_.nullspace.regularization);
  ...
  for (int i = 0; i < num_joints_; i++) {
    tau_nullspace[i] =
        std::max(-params_.nullspace.max_tau,
...
CallbackReturn TorqueFeedbackController::on_init() {
  ...
  friction_fp1_ = Eigen::Map<const Eigen::VectorXd>(
      params_.friction.fp1.data(), params_.friction.fp1.size());
  friction_fp2_ = Eigen::Map<const Eigen::VectorXd>(
      params_.friction.fp2.data(), params_.friction.fp2.size());
  friction_fp3_ = Eigen::Map<const Eigen::VectorXd>(
      params_.friction.fp3.data(), params_.friction.fp3.size());
...
CallbackReturn TorqueFeedbackController::on_configure(
  ...
  std::set<std::string> allowed_joint_types = {"JointModelRZ",
    ...
    if (not allowed_joint_types.count(model_.joints[joint_id].shortname())) {
      RCLCPP_ERROR_STREAM(get_node()->get_logger(),
                          "Joint type "
                              ...
                              << "). Continuous joints are not implemented yet "
                                 "for torque feedback controller. "
