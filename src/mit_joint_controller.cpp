...
MITJointController::update(const rclcpp::Time &time,
  ...
  timing_stats_.state_read.add_sample(
      timespec_diff_us(section_start, section_end));
  ...
  clock_gettime(CLOCK_MONOTONIC, &section_end);
  timing_stats_.velocity_filter.add_sample(
      timespec_diff_us(section_start, section_end));
  ...
  clock_gettime(CLOCK_MONOTONIC, &section_end);
  timing_stats_.target_parse.add_sample(
      timespec_diff_us(section_start, section_end));
  ...
  if (params_.oscillation_detection.enabled) {
    ...
    clock_gettime(CLOCK_MONOTONIC, &section_end);
    timing_stats_.oscillation_detect.add_sample(
        timespec_diff_us(section_start, section_end));
  }
  ...
  clock_gettime(CLOCK_MONOTONIC, &section_end);
  timing_stats_.control_compute.add_sample(
      timespec_diff_us(section_start, section_end));
  ...
  clock_gettime(CLOCK_MONOTONIC, &section_end);
  timing_stats_.command_write.add_sample(
      timespec_diff_us(section_start, section_end));
  ...
  if (params_.log.enabled && csv_logger_) {
    ...
    log_data.loop_duration_ms =
        timespec_diff_us(loop_start, loop_end_for_csv) * 0.001;

    csv_logger_->logData(log_data);

    clock_gettime(CLOCK_MONOTONIC, &section_end);
    timing_stats_.csv_logging.add_sample(
        timespec_diff_us(section_start, section_end));
  }
...
CallbackReturn MITJointController::on_configure(
  ...
  if (get_node()->has_parameter("robot_description")) {
    ...
    if (!robot_description.empty()) {
      RCLCPP_INFO(get_node()->get_logger(),
                  "Got robot_description parameter from controller node "
                  "(length: %zu bytes)",
                  robot_description.size());
    }
  }

  // Fallback: subscribe to robot_description topic if not found on controller
  // node
  if (robot_description.empty()) {
    RCLCPP_WARN(
        get_node()->get_logger(),
    ...
    auto sub = get_node()->create_subscription<std_msgs::msg::String>(
        params_.robot_description_topic,
    ...
    if (urdf_future.wait_for(std::chrono::seconds(5)) ==
        std::future_status::timeout) {
      RCLCPP_ERROR(get_node()->get_logger(), "Timeout waiting for %s topic",
                   params_.robot_description_topic.c_str());
    ...
    if (robot_description.empty()) {
      RCLCPP_ERROR(get_node()->get_logger(),
                   "robot_description from topic is empty");
      return CallbackReturn::ERROR;
    }
  ...
  RCLCPP_INFO_STREAM(get_node()->get_logger(),
                     "Loaded full robot model with "
                         << full_model.njoints << " joints (nv="
                         << full_model.nv << ", nq=" << full_model.nq << ")");
  ...
  if (num_joints == static_cast<size_t>(full_model.nv)) {
    ...
    RCLCPP_INFO_STREAM(get_node()->get_logger(),
                       "Building reduced model with "
                           << num_joints << " controlled joints out of "
                           << full_model.nv);

    // Create list of joint IDs to keep
    std::vector<pinocchio::JointIndex> joints_to_lock;
    for (pinocchio::JointIndex joint_id = 1;
         joint_id < full_model.joints.size(); ++joint_id) {
    ...
    RCLCPP_INFO_STREAM(get_node()->get_logger(), "Locking "
                                                     << joints_to_lock.size()
                                                     << " uncontrolled joints");
    ...
    if (static_cast<size_t>(model_.nv) != num_joints) {
      RCLCPP_ERROR(get_node()->get_logger(),
                   "Reduced model nv (%d) does not match number of controlled "
                   "joints (%zu). "
...
void MITJointController::parse_target_joint_() {
  ...
  // Require joint names for partial/full updates (O(1) lookup via map)
  if (msg->name.empty()) {
    RCLCPP_WARN_THROTTLE(
        get_node()->get_logger(), *get_node()->get_clock(), 5000,
        "Received target_joint without joint names (ignoring). "
        "Please provide joint names for partial or full updates.");
    new_target_ = false;
    return;
  }

  Eigen::VectorXd q_target_new = q_target_; // Start with current target
  bool any_position_updated = false;

  // Use O(1) lookup to update only specified joints
  for (size_t msg_idx = 0; msg_idx < msg->name.size(); ++msg_idx) {
    const std::string &joint_name = msg->name[msg_idx];

    // O(1) lookup in joint name map
    auto it = joint_name_to_index_.find(joint_name);
    if (it != joint_name_to_index_.end()) {
      size_t ctrl_idx = it->second;

      // Update position if provided
      if (msg_idx < msg->position.size()) {
        q_target_new[ctrl_idx] = msg->position[msg_idx];
        any_position_updated = true;
      }

      // Update velocity if provided
      if (msg_idx < msg->velocity.size()) {
        dq_target_[ctrl_idx] = msg->velocity[msg_idx];
      }
    } else {
      RCLCPP_WARN_THROTTLE(get_node()->get_logger(), *get_node()->get_clock(),
                           5000,
                           "Received target for unknown joint: %s (ignoring)",
                           joint_name.c_str());
    }
  }

  // Apply position updates with smoothing if enabled
  if (any_position_updated) {
    if (params_.target_smoothing.enabled) {
...
void MITJointController::log_timing_statistics_() {
  RCLCPP_INFO(
      ...
      "target_parse.avg=%ldus | control_compute.avg=%ldus "
      "control_compute.max=%ldus | "
      "oscillation.avg=%ldus | command_write.avg=%ldus | "
      "csv_log.avg=%ldus | cycles=%zu",
      timing_stats_.total_loop.avg_us(), timing_stats_.total_loop.std_us(),
      timing_stats_.total_loop.max_us, timing_stats_.state_read.avg_us(),
      ...
      timing_stats_.command_write.avg_us(), timing_stats_.csv_logging.avg_us(),
